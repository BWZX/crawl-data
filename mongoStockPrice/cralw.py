        ...
        while trade_days_index<len(trade_days):
            ...
            if tem.isOpen:
                if not df_D[df_D.date==tem.calendarDate].empty:
                    ...
                    ll=list(security.objects.raw({'date':tem.calendarDate, 'code':sto}))
                    if not ll:
                        security(tem.calendarDate, lastprice, lastprice,lastprice,lastprice, 0, sto, config.StocksList[sto], 'D').save()
                    else:
                        security.objects.raw({'date':tem.calendarDate, 'code':sto}).update(
...
def CrawlContinue():
    ...
    for i in range(len(todayall)):
        ...
        if tem.open<0.001:
            ...
            if not ll:
                security(today, lastprice, lastprice,lastprice,lastprice, 0, tem.code, tem['name'], 'D').save()
            else:
            ...
            ll=list(security.objects.raw({'date':today, 'code':tem.code}))
            if not ll:
                security(today, tem.open, tem.trade,tem.high,tem.low, tem.volume, tem.code, tem['name'], 'D').save()
            else:
                security.objects.raw({'date':today, 'code':tem.code}).update(
